...
The Jakarta Persistence specification defines JPQL (a pseudo-OO query language, with SQL-like syntax), for selecting objects from the datastore. 
...
NOTE: DataNucleus Jakarta also allows _RIGHT_ OUTER JOIN, though this is not part of the Jakarta Persistence spec.
...
In strict Jakarta the entity name cannot be a _MappedSuperclass_ entity name. That is, if you have an abstract superclass that is persistable, 
...
You are unlikely to find this supported in other Jakarta implementations, but then maybe that's why you chose DataNucleus?
...
In strict Jakarta you cannot join to another "root" element. That is, you define JOIN syntax to the following element *along a relation*
...
In strict Jakarta you cannot join to an embedded element class (of an embeddable). With DataNucleus you can do this, and hence form queries
using fields of the embeddable (not available in most other Jakarta providers). See this example, where class _Person_ has a Collection 
...
NOTE: This is only supported where the RDBMS supports it (H2, HSQLDB, PostgreSQL, DB2, Oracle, Derby, Firebird, SQLServer v11+, SQLite v3.30+).
...
image:../images/nucleus_extensionpoint.png[link=../extensions/extensions.html#query_method_evaluator]
...
|SIGN(num_field)
|Returns the sign (+1, 0, -1) of a numeric field
|icon:check[]
|icon:check[]
...
|ATAN2(num_field, num_field)
|Returns the arc-tangent of numeric fields
|icon:times[]
|icon:check[]
...
|COT(num_field)
|Returns the cotangent of a numeric field
|icon:times[]
|icon:check[]

|COSH(num_field)
|Returns the hyperbolic cosine of a numeric field
|icon:times[]
|icon:check[]

|SINH(num_field)
|Returns the hyperbolic sine of a numeric field
|icon:times[]
|icon:check[]

|TANH(num_field)
|Returns the hyperbolic tangent of a numeric field
|icon:times[]
|icon:check[]
...
NOTE: in the Jakarta Persistence spec they allow a user to interchangeably use "p.addresses" to refer to the _value_ of the Map. Whilst DataNucleus supports
...
With Jakarta Persistence you always query objects of the candidate type in the datastore. DataNucleus extends this and allows you to provide
...
((org.datanucleus.api.jakarta.JakartaQuery)query).getInternalQuery().setCandidates(myCandidates);
...
A special case, where you don't have a result class but want to easily extract multiple columns in the form of a *Tuple* Jakarta provides a special class 
_jakarta.persistence.Tuple_ to supply as the result class in the above call. From that you can get hold of the column aliases, and their values.
...
NOTE: DataNucleus also supports specifying this using `@NamedQuery` annotation in non-Entity classes. This is beyond the Jakarta Persistence spec, but is very useful in real applications.
...
-----
Query q = em.createQuery("SELECT p FROM Product p WHERE ...");
((org.datanucleus.api.jakarta.JakartaQuery)q).saveAsNamedQuery("MyQuery");
-----
...
String sql = (String)((org.datanucleus.api.jakarta.JakartaQuery)q).getNativeQuery();
-----
...
The Jakarta Persistence specification defines a mode of JPQL for deleting objects from the datastore.
...
The Jakarta Persistence specification defines a mode of JPQL for updating objects in the datastore. 
...
In strict Jakarta you cannot use a subquery in the UPDATE clause. With DataNucleus Jakarta you can do this so, for example, you can set a field to the result of a subquery.
...
By default DataNucleus allows some extensions in syntax over strict JPQL (as defined by the Jakarta Persistence spec). To allow only strict JPQL you can do as follows

[source,java]
-----
Query query = em.createQuery(...);
query.setHint("datanucleus.query.jpql.strict", "true");
-----
