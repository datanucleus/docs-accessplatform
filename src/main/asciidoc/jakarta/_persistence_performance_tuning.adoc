...
Jakarta Persistence provides properties for generating the schema at startup, and DataNucleus also provides some of its own 
...
* Use *datanucleus.RetainValues*=true. This is the default for Jakarta Persistence operation and will ensure that after commit the fields of the object retain their values 
...
This can have an impact if you are then detaching those objects. With Jakarta Persistence the maximum fetch depth is -1 (unlimited).
So with Jakarta Persistence you ought to set it to the extent that you want to detach, or better still make use of link:persistence.html#entity_graphs[Jakarta Entity Graphs] 
