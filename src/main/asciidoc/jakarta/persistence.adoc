[[persistence]]
= Jakarta Persistence Guide (v{version})
:_basedir: ../
:_imagesdir: images/
:jakarta:


We saw in link:mapping.html[Jakarta Mapping Guide] how to map classes for persistence with the Jakarta Persistence API.
In this guide we will describe the Jakarta Persistence API itself, showing how to persist, update and delete objects from persistence.

You should familiarise yourself with the http://www.datanucleus.org/javadocs/jakarta.persistence/3.0/[Jakarta 3.0 Javadocs].
...
include::_persistence_locking.adoc[leveloffset=+1]
