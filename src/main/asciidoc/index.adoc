...
DataNucleus supports both, fully, and also provides support for a link:rest/rest.html[REST API].

* *APIs Supported* : link:jdo/persistence.html[JDO 3.2], link:jpa/persistence.html[JPA 2.1+], link:rest/rest.html[REST]
* *Mapping Supported* : link:jdo/mapping.html#class[JDO], link:jpa/mapping.html#class[JPA]
...
link:datastores/datastores.html#googlestorage[GoogleStorage], link:datastores/datastores.html#ldap[LDAP], link:datastores/datastores.html#neodatis[Neodatis]
...
Documentation for all aspects of DataNucleus support for the JDO and JPA APIs are shown below, including PDF downloads.
...
* *Persistence API* : both persistence APIs are very similar. JDO provides more options and control though for basic persistence and retrieval there are differences only in the methods
...
Remember that you can use JPA metadata with the JDO persistence API, and vice-versa.
...
We provide an link:extensions/extensions.html[Extensions Guide] icon:file-pdf-o[link="./extensions/extensions.pdf"] describing the points where the DataNucleus codebase 
