...
*DataNucleus v6.0 is aiming at Java11+. It is in early development. If you want to get involved, now would be the time to register your interest.*
...
DataNucleus v{version} supports all 3, fully, and also provides support for a link:rest/rest.html[REST API].

* *APIs Supported* : link:jdo/persistence.html[JDO 3.2], link:jpa/persistence.html[JPA 2.2+], link:jakarta/persistence.html[Jakarta 3.0+], link:rest/rest.html[REST]
* *Mapping Supported* : link:jdo/mapping.html[JDO], link:jpa/mapping.html[JPA], link:jakarta/mapping.html[Jakarta]
...
In addition to the JDO, JPA and Jakarta Persistence API support, we also provide a link:rest/rest.html[REST API Guide] icon:file-pdf[link="./rest/rest.pdf"] 
...
* *Target datastore* : JDO is designed for all datastores, whereas JPA/Jakarta are just designed around RDBMS and explicitly uses RDBMS/SQL terminology. 
...
DataNucleus JPA/Jakarta adaptable to non-RDBMS features
...
* *Persistence API* : both persistence APIs are very similar. JDO provides more options and control though for basic persistence and retrieval, and there are 
...
JPQL is more closely aligned with RDBMS concepts and SQL. If you just want SQL then you can use any of JDO, JPA, Jakarta since both provide this
* *Fetch Control* : do you need full control of what is fetched, and when ? JDO provides Fetch Groups, whereas JPA/Jakarta provide Entity Graphs (a subset of Fetch Groups). 
Use JDO if full fetch groups is an important factor for your design, otherwise any
...
Remember that you can use JPA/Jakarta metadata with the JDO persistence API, and vice-versa. You are more likely to find developers with JPA/Jakarta experience.
...
* Define the persistence *mapping* for your classes, using either JDO annotations, JDO XML metadata, JPA annotations, JPA XML metadata, 
Jakarta annotations or Jakarta XML metadataor a mixture of these.
