...
* *APIs Supported* : link:jdo/persistence.html[JDO 3.2], link:jakarta/persistence.html[Jakarta 3.0+], link:jpa/persistence.html[JPA 2.2+], link:rest/rest.html[REST]
...
In addition to the JDO, Jakarta Persistence and JPA API support, we also provide a link:rest/rest.html[REST API Guide] icon:file-pdf[link="./rest/rest.pdf"] 
...
* *Target datastore* : JDO is designed for all datastores, whereas Jakarta/JPA are just designed around RDBMS and explicitly uses RDBMS/SQL terminology. 
...
DataNucleus Jakarta/JPA adaptable to non-RDBMS features
...
JPQL is more closely aligned with RDBMS concepts and SQL. If you just want SQL then you can use any of JDO, Jakarta/JPA since all provide this
* *Fetch Control* : do you need full control of what is fetched, and when ? JDO provides Fetch Groups, whereas Jakarta/JPA provide Entity Graphs (a subset of Fetch Groups). 
...
Remember that you can use Jakarta/JPA metadata with the JDO persistence API, and vice-versa. You are more likely to find developers with Jakarta/JPA experience.
...
* Define the persistence *mapping* for your classes, using either JDO annotations, JDO XML metadata, Jakarta annotations, Jakarta XML metadata, 
