...
JDO has always accepted Metadata in XML format. As described in the link:metadata.html[Metadata Overview] this has to be contained in files with
...
You can provide JDO metadata, or alternatively ORM metadata, but with virtually identical format. 
This page defines the format of the XML Metadata. Here is an example header for *package.jdo* files with 
http://xmlns.jcp.org/xml/ns/jdo/jdo_3_0.xsd[JDO XSD] specification
