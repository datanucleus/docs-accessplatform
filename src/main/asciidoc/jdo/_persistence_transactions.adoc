...
With DataNucleus JDO this is enabled by default (see the 2 properties *datanucleus.nontransactionalRead*, *datanucleus.nontransactionalWrite* set to _true_, the default). 
...
NOTE: Calling _flush()_ when not transactional will do nothing.
...
This can be specified globally via the persistence property *datanucleus.transactionIsolation* (javax.jdo.option.TransactionIsolationLevel).
