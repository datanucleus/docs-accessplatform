...
Firstly, you will need the following for _compile time_ building against the JDO API.
...
If running your app using Maven "exec" plugin then the runtime specification may not be needed.
...
If you want to use an earlier version then you need to add exclusions to the `maven-datanucleus-plugin`
