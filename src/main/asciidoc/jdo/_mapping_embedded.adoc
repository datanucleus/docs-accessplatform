...
NOTE: by default the embedded objects cannot have inheritance. Inheritance in embedded objects is only supported for RDBMS and MongoDB, 
and only with all inherited fields stored in the same table with a discriminator defined in the metadata of the embedded type.
