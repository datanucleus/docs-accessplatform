...
JDO 3.2 introduces a way of performing queries using a JDOQLTypedQuery 
http://www.datanucleus.org/javadocs/3.2/javax/jdo/JDOQLTypedQuery.html[image:../images/javadoc.png[Javadoc]] API, that copes with refactoring of classes/fields.
...
JDOQLTypedQuery<Product> tq = pm.newJDOQLTypedQuery(Product.class);
QProduct cand = QProduct.candidate();
List<Object[]> results = tq.filter(cand.value.lt(40.00).and(cand.name.startsWith("Wal")))
      .orderBy(cand.name.asc())
      .result(false, cand.name, cand.value)
      .executeResultList();
-----
...
-----
JDOQLTypedQuery<Product> tq = pm.newJDOQLTypedQuery(Product.class);
Object results = tq.result(false, QProduct.candidate().max(), QProduct.candidate().min())
      .executeResultUnique();
-----
...
List<Inventory> results = tq.filter(cand.products.contains(var).and(var.name.startsWith("Wal")))
      .executeList();
-----
...
List<Product> results = tq.filter(cand.value.lt(tqsub.selectUnique(candsub.value.avg())))
      .executeList();
-----
