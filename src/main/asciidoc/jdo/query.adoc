...
NOTE: For some datastores additional query languages may be available specific to that datastore - please check the link:../datastores/index.html[datastores documentation].



There are 2 categories of queries with JDO :-

* *Programmatic Query* where the query is defined using the link:query.html#query_api[JDO Query API].
* *Named Query* where the query is link:query.html#named[defined in MetaData and referred to by its name at runtime].
