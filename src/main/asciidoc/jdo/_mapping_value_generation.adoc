...
WARNING: Which generation strategy is used internally will be JDO provider dependent. 
If you want to be portable and independent of a JDO providers internals you should likely avoid use of `native`.
...
the column type for the PK of the base table will be defined as `AUTO_INCREMENT` / `IDENTITY` / `SERIAL` (dependent on the RDBMS) and all subtables will 
...
This method requires a _sequence_ table in the database and creates one if doesn't exist.
...
the form _LLLLLLLL-MMMM-HHHH-CCCC-NNNNNNNNNNNN_ where 'L', 'M', 'H', 'C' and 'N' are the DCE UUID fields
