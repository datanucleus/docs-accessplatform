...
As long as your persistable class is _detachable_ (see the link:mapping.html[mapping guide]) then you can _detach_ objects of that type. 
...
=== Detaching objects managed by a PersistenceManager at close

A further alternative is to set the persistence property *datanucleus.detachOnClose* to true so that when you close your _PersistenceManager_ then all objects currently managed
will be detached (when they are detachable).
