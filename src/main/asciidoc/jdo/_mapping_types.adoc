...
type into a String or Long. See also link:../extensions/extensions.html#java_type[the Java Types plugin-point]
...
and now in our PersistenceCapable class we mark the _myField_ as being converted using this converter
...
This is all well and good, it will map this type on to a single String-based column now. There remains one problem, it is still thinking that this type should be _wrapped_ (to intercept
calls to the Collection methods). Since our type hides all of that, we want to disable this. So we update the mapping to be
...
Please refer to the link:../extensions/extensions.html#java_type[java_type extension]. In which case you could omit this _is-second-class_ metadata.
