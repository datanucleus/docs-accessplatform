...
See also link:../extensions/extensions.html#java_type[the Java Types plugin-point].
...
By default if a type extends Collection/Map then it will be marked as _second class_, and that DataNucleus will treat it as Collection that should be _wrapped_ (see above for Collection/Map).
...
This is all well and good, it will map this type on to a single String-based column now. 
There remains one problem, it is still thinking that this type should be _wrapped_ (to intercept calls to the Collection methods). 
Since our type hides all of that, we want to disable this. So we update the mapping to be
...
so now it is not attempted to be wrapped and will persist and retrieve fine.


NOTE: If you want your extension of Collection/Map to be managed as a _mutable_second class type then you will need to provide a _wrapper_ class for it.
Please refer to the link:../extensions/extensions.html#java_type[java_type extension] for how to support this. In which case you could omit this _is-second-class_ metadata.
