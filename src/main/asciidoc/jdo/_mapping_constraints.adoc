...
This will mean that DataNucleus will create an index in the datastore for the field and the index will have the name _BOOKING_TYPE_INDEX_ (for datastores that support using named indexes). 
...
Please be aware that JDO provides <a href="cascading.html#deletion_dependent[Dependent Fields] as a way of allowing cascade deletion. 
