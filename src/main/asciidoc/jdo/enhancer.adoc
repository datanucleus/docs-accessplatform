...
Rather than requiring that a user add this themself, we provide an link:#DataNucleus_Enhancer[enhancer] that will modify your compiled classes to implement all required methods.
...
* Post-compilation
** xref:#maven[Using Maven via the DataNucleus Maven plugin]
** xref:#ant[Using Ant]
** xref:#manual[Manual invocation at the command line]
** link:guides/eclipse.html[Using the Eclipse DataNucleus plugin]
* At runtime
** xref:#runtime[Runtime Enhancement]
** xref:#api[Programmatically via an API]
