...
Note that the types in *bold* are JDO standard types. Any others are DataNucleus extensions and, as always, link:../datastores/index.html[check the specific datastore docs]
...
Lets take the same classes as we have in the linke:mapping.html#one_many_map_fk_bi_key[1-N Relationships (FK)]. 
