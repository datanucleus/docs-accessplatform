...
image:../images/javadoc.png[Javadoc, link=http://www.datanucleus.org/javadocs/javax.jdo/3.2/javax/jdo/PersistenceManagerFactory.html]. 
...
TIP: Always close your _PersistenceManagerFactory_ / _PersistenceManager_ objects after you have finished with them.
...
The first property specifies to use the DataNucleus implementation, and the following 3 properties (javax.jdo.option.Connection???) define the datastore that it should connect to. 
...
This is the JDO XML Metadata file (`package.jdo`) (*not* the ORM XML Metadata file)
...
<jdoconfig xmlns="https://db.apache.org/jdo/xmlns/jdoconfig"
    ...
    xsi:schemaLocation="https://db.apache.org/jdo/xmlns/jdoconfig
        https://db.apache.org/jdo/xmlns/jdoconfig_3_2.xsd" version="3.2">
...
|The name of the PMF implementation. This is only required if you have more than one JDO implementation in the CLASSPATH, and otherwise defaults to _org.datanucleus.api.jdo.JDOPersistenceManagerFactory_.
...
When using RDBMS you can select from DBCP2, C3P0, HikariCP, BoneCP, etc. You must have the 3rd party jars in the CLASSPATH to use these options.
...
Refer to link:../extensions/extensions.html#store_datastoreidentity[Datastore Identity] image:../images/nucleus_extensionpoint.png[]
...
|Order of creating the schema, whether scripts or metadata
{scripts, scripts-then-metadata, *metadata*, metadata-then-scripts}
...
|Order of dropping the schema, whether scripts or metadata
{scripts, scripts-then-metadata, *metadata*, metadata-then-scripts}
...
|Select the default transaction isolation level for identity generation. Must have *datanucleus.valuegeneration.transactionAttribute* set to _New_
...
{*None*, Classes, MetaData, SchemaTable}
...
|Whether to add unique constraints to the element table for a map inverse (FK) field.
{*true*, false}
...
|If enabled will, upon a request to load a field, check for any unloaded BASIC (non-relation) fields and will load them in the same SQL call.
Be careful using this if you have some BLOB/CLOB fields that you don't want to load like this
{true, *false*}

|datanucleus.rdbms.autoFetchUnloadedFKs
|If enabled will, upon a request to load a field, check for any unloaded 1-1/N-1 fields with a FK at this side and will load them in the same SQL call avoiding
a join to a related table. The value will then be stored in the owning StateManager for when that field is ever accessed, making the load more efficient.
{true, *false*}
...
NOTE: The L2 cache is a DataNucleus image:../images/nucleus_extensionpoint.png[link=../extensions/extensions.html#cache_level2] allowing you to provide your own cache where you require it. 
...
The _DataStoreCache_ interface image:../images/javadoc.png[Javadoc, link=http://www.datanucleus.org/javadocs/javax.jdo/3.2/javax/jdo/datastore/DataStoreCache.html]
