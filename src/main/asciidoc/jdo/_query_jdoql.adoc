...
List<Person> results = tq.filter(cand.lastName.eq("Jones").and(cand.age.lt(tq.intParameter("age_limit"))))
...
link:../extensions/extensions.html#query_method_evaluators[DataNucleus plugin point]

image:../images/nucleus_plugin.png[]
Please note that you can easily add support for other methods with RDBMS datastore using this 
link:../extensions/extensions.html#rdbms_sql_method[DataNucleus plugin point]
...
|trimLeft()
|Returns a trimmed version of the string (trimmed for leading spaces). *Only on RDBMS*
|icon:times[]
|icon:check[]

|trimRight()
|Returns a trimmed version of the string (trimmed for trailing spaces). *Only on RDBMS*
|icon:times[]
|icon:check[]
|===
...
In addition, DataNucleus JDO provides support for a number of specialised link:query.html#jdoql_geospatial_methods[Geospatial methods].
...
with the Declarative API you would define explicit parameters like this
[source,java]
-----
...
=== Controlling the join type for variables

image:../images/nucleus_plugin.png[]

NOTE: Applicable to RDBMS

In all situations we aim for DataNucleus JDOQL implementation to work out the right way of linking a variable into the query, whether this is via a join (INNER, LEFT OUTER), 
or via a subquery. As you can imagine this can be complicated to work out the optimum for all situations so with that in mind we allow (for a limited number of situations) 
the option of specifying the join type. This is achieved by setting the query extension *datanucleus.query.jdoql.{varName}.join* to the required type. 
...
NOTE: If you always fully-qualify candidate, variable and parameter types then there is no need to specify any _imports_ (just like in Java).
...
SELECT FROM org.datanucleus.Employee WHERE salary > 
    (SELECT avg(e.salary) FROM org.datanucleus.Employee e)
-----
...
-----
SELECT FROM org.datanucleus.Employee WHERE salary > 
    (SELECT avg(e.salary) FROM org.datanucleus.Employee e WHERE e.lastName == this.lastName)
-----
...
A subquery is effectively a Collection, so you have access to the normal methods of a Collection to use on the subquery. Here are as couple of examples
...
Similarly you can make use of _size()_.
...
include::_query_jdoql_geospatial_methods.adoc[leveloffset=+1]
