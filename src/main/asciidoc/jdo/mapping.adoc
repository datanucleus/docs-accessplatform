...
NOTE: Whilst the JDO spec provides for you to specify your mapping information using JDO metadata (annotations, or JDO/ORM XML Metadata, or via the Metadata API), 
it also allows you the option of using JPA metadata (annotations, `orm.xml`). This is provided as a way of easily migrating across to JDO from JPA, for example. 
