...
DataNucleus JDO provides an implementation of this JPA _standard_, allowing you, the user, to persist your
...
These include popular map stores such as Cassandra and HBase, the Neo4j graph store, spreadsheets in Excel or OpenOffice formats, 
...
* Use of JDO requires an link:persistence.html#pmf[PersistenceManagerFactory] to access the datastore.
* The persistence itself is controlled by an link:persistence.html#pm[PersistenceManager] and each object to be persisted will
...
include::tutorial_rdbms.adoc[leveloffset=+1]
