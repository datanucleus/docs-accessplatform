...
The default (JDO supported) index specifies ascending index columns. Most RDBMS these days support specifying the ordering of index columns.
You can control this as follows

[source,xml]
-----
<class name="Booking">
    <index name="MY_OTHER_BOOKING_IDX" unique="true">
        <column name="NAME"/>
        <column name="NUMBER"/>
...
[source,java]
-----
@Index(name="MY_OTHER_BOOKING_IDX" extensions={@Extension(vendorName="datanucleus", key="index-column-ordering", value="ASC,DESC")})
-----

which will create an index with `CREATE UNIQUE INDEX MY_OTHER_BOOKING_IDX ON BOOKING ("NAME" ASC,"NUMBER" DESC)`
...
[source,java]
-----
@Index(name="MY_BOOKING_INDEX" extensions={@Extension(vendorName="datanucleus", key="index-type", value="HASH")})
-----
...
A final extension is where you just want to dump an amount of SQL onto the end of the `CREATE INDEX` statement.
