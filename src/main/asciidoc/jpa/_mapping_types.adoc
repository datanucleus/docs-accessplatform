...
NOTE: The `AttributeConverter` objects shown here are *stateless*. That is, they do not require CDI injection by DataNucleus.


NOTE: JPA 2.2 allows `AttributeConverter` objects to be CDI injectable. DataNucleus supports this, but clearly will only be usable in a CDI environment.
