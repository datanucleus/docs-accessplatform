...
* *1* (read-uncommitted) : dirty reads, non-repeatable reads and phantom reads can occur
* *2* (read-committed) : dirty reads are prevented; non-repeatable reads and phantom reads can occur
* *4* (repeatable-read) : dirty reads and non-repeatable reads are prevented; phantom reads can occur
...
Alternatively if you prefer String-based input
