...
DataNucleus provides an extension to overcome this gap in the JPA spec. Here we make use of a DataNucleus extension annotation *@JdbcType*
[source,java]
-----
public class Payment
{
...
-----
So we defined the JDBC type that this field will use (rather than the default of VARCHAR). Now we turn our attention to the _length_, _precision_ and _scale_.
[source,xml]
-----
<entity name="Payment">
    <attributes>
...
-----
You could alternatively specify these using annotations should you so wish. So we have defined TRANSFER_REF to use VARCHAR(255) column type, CURRENCY to use (VAR)CHAR(3) column type, 
and AMOUNT to use DECIMAL(10,2) column type.
