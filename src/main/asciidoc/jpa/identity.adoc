...
* xref:identity.html#application[Application Identity] : a field, or several fields of the persistable type are assigned as being (part of) the primary key.
* xref:identity.html#datastore[Datastore Identity] : a surrogate column is added to the persistence of the persistable type, and objects of this type are identified by
the class plus the value in this surrogate column. *DataNucleus Extension*
* xref:identity.html#nondurable[Nondurable Identity] : the persistable type has no identity as such, so the only way to lookup objects of this type would be
via query for values of specific fields. This is useful for storing things like log messages etc. *DataNucleus Extension*

A further complication is where you use _application identity_ but one of the fields forming the primary key is a relation field. This is known as xref:identity.html#derived[Derived Identity].

[[application]]
== Application Identity

With *application identity* you are taking control of the specification of id's to DataNucleus. Application identity requires a primary key class 
_(unless using SingleFieldIdentity, where one is provided for you)_, and each persistent capable class may define a different class for its primary 
...
* link:value_generation.html[Identity Generation Guide] - strategies for generating ids
...
Note that the types in *bold* are JPA standard types. Any others are DataNucleus extensions and, as always, link:../datastores/index.html[check the specific datastore docs] 
...
link:value_generation.html[value-strategy] on that field.
...
Please note that if one of the fields that comprises the primary key is in itself an entity then you have xref:identity.html#derived[Derived Identity]
...
While JPA defines support for xref:identity.html#application[application identity] only, DataNucleus also provides support for *datastore identity*. 
...
to be able to access the identity then you should be using xref:identity.html#application[application identity]. 
...
* xref:identity.html#derived_1_1_uni[1-1 unidirectional]
* xref:identity.html#derived_1_N_coll_bi[1-N collection bidirectional using ForeignKey]
* xref:identity.html#derived_1_N_map_bi[1-N map bidirectional using ForeignKey (key stored in value)]
...
Lets take the same classes as we have in the link:one_to_one.html[1-1 Relationships].
...
image:../images/orm/relationship_1_1_compound_class.jpg[]
...
image:../images/orm/relationship_1_1_compound_db.jpg[]
...
* See also the xref:identity.html#application_primarykey[general instructions for Primary Key classes]
...
Lets take the same classes as we have in the link:orm/one_to_many_collection.html#fk_bi[1-N Relationships (FK)].
...
image:../images/orm/relationship_1_N_compound_class.jpg[]
...
image:../images/orm/relationship_1_N_compound_db.jpg[]
...
Lets take the same classes as we have in the link:orm/one_to_many_map.html#fk_bi_key[1-N Relationships FK].
...
*ACCOUNT*. In our example here we want to have the primary-key to *ACCOUNT* to _include_ the foreign-key. To do this we need to modify the classes slightly, 
adding primary-key fields to both classes, and use "application-identity" for both.

image:../images/orm/relationship_1_N_bi_map_fk_simple_pc_class.jpg[]
In addition we need to define primary key classes for our *Account* and *Address* classes
...
So now we have the following datastore schema
image:../images/orm/relationship_1_N_compound_map_db.jpg[]
