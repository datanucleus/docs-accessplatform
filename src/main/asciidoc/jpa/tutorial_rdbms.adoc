...
* The *datanucleus-core*, *datanucleus-api-jpa* and *datanucleus-rdbms* JARs
...
This information is used _either_ to match up to an existing schema, _or_ is used to generate a new schema (see #xref:tutorial_rdbms.html#step7[Step 7]).
So we define a file `_META-INF/orm.xml_` at the root of the CLASSPATH.
