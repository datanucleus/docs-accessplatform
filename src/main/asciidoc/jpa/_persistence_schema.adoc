...
* At runtime, link:#schema-generation[as a one-off generate-schema step]. This is the recommended option since it is standard in JPA2.1
...
If you want to create the schema ("tables"+"columns"+"constraints") during the persistence process, the property 
...
This can be useful for example where you are trying to map to an existing schema and want to verify that you've got the correct MetaData definition.</p>
...
*Important : if using SchemaTool with a persistence-unit make sure you omit _javax.persistence.schema-generation_ properties
...
* <a href="guides/eclipse.html#schematool[Invoke it using the DataNucleus Eclipse plugin]
...
for the plugin (in your _pom.xml_). For example
...
*datanucleus.ConnectionDriverName*, *datanucleus.ConnectionURL*, *datanucleus.ConnectionUserName*, and *datanucleus.ConnectionPassword* 
