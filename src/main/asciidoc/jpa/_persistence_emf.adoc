...
In this example we have defined 2 _persistence-units_. 
...
* *shared-cache-mode* - Defines the way the L2 cache will operate. ALL means all entities cached. NONE means no entities will be cached. 
ENABLE_SELECTIVE means only cache the entities that are specified. DISABLE_SELECTIVE means cache all unless specified. UNSPECIFIED leaves it to DataNucleus.
...
* *properties* - properties defining the persistence factory to be used. Please refer to link:#emf_properties[EMF Properties Guide] for details
* *exclude-unlisted-classes* - Whether to restrict the unit to just classes specified in the unit itself (including jars and mapping-files)
...
See also link:#cache_level2[Cache docs].
...
See link:../extensions/extensions.html#datastoreidentity[Datastore Identity extension] for details.
...
|Name of the type of Level 1 cache to use. Defines the backing map. See also link:#level1_cache[L1 Cache docs]
...
See also link:#cache_level2[L2 Cache docs]
