...
* *Specify the connection URL/userName/password/driverName* and it will internally create a DataSource for this URL (or equivalent for non-RDBMS). 
...
|The name of the (JDBC) driver to use for the DB (for RDBMS only, and not needed for JDBC 4+ drivers).
...
|Whether to use optimistic locking (link:#locking_optimistic[Optimistic locking]).
{*true*, false}
