...
This will create 2 tables in the database, one for *Address* (including an _ACCOUNT_ID_ to link to the `ACCOUNT` table), and one for *Account*. 
...
We have defined the same foreign key for the 2 collections "ACCOUNT_ID_OID", The important step is then to define the 2 DataNucleus _extension_ tags (@SharedRelation annotation). 
