...
=== FETCH JOIN of multi-valued field

image:../images/nucleus_extension.png[]

For RDBMS, if you have a FETCH JOIN to a multi-valued field of the candidate, DataNucleus will perform the basic query in one SQL statement,
and then will invoke an extra SQL of the form _SELECT {relatedObject columns} FROM RELATED_TBL WHERE EXISTS (restrict to the candidate objects involved).
This is referred to as *bulk fetch* and avoids the "1+N query problem".

WARNING: This only applies to fields of the candidate that are FETCH JOINed. If you have a FETCH JOIN from a related object then this will not currently be fetched.

NOTE: If you don't want multi-valued fields to be fetched in this way, either don't mark them to be fetched or set the query hint *datanucleus.rdbms.query.multivaluedFetch* to _none_ 
...
=== Candidate that is @MappedSuperclass
...
=== Control over INNER/OUTER join for implicit joins
...
Please note that you can easily add support for other functions for evaluation "in-memory" using this link:../extensions/extensions.html#query_method_evaluator[DataNucleus plugin point]
