...
For RDBMS datastores any multi-valued (1-N/M-N) field will be _bulk-fetched_ if it is defined to be EAGER or has a _FETCH JOIN_, or is placed in the current EntityGraph. 
...
Please note that temporal literals are specified using _JDBC escape syntax_ (see the JDBC spec for full details) in String form, namely
